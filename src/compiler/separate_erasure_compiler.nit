...
class SeparateErasureCompiler
	...
	do
		...
		if mtype.is_c_primitive or mtype.mnominal.mclass.name == "Pointer" then
			...
			v.add("\}")
			return
		else if mtype.mnominal.mclass.kind == extern_kind and mtype.mnominal.mclass.name != "NativeString" then
...
class SeparateErasureCompilerVisitor
	...
	do
		...
		self.require_declaration("NEW_{ret.mnominal.mclass.c_name}")
		self.ret(self.new_expr("NEW_{ret.mnominal.mclass.c_name}({arguments[1]})", ret_type))
	end
end
