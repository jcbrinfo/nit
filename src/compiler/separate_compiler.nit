			...
			for e in meth_layout do
				if e == null then
					...
					if mtype.may_have_mproperty(mainmodule, e) then
						...
						# Because of generic classes (combined with subsets),
						# there are cases where a subclass does not inherit all
						# (implicit) properties of its superclasses.
						meth_table.add null
					end
					...
					if mtype.may_have_mproperty(mainmodule, e.mproperty) then
						...
						# Because of generic classes (combined with subsets),
						# there are cases where a subclass does not inherit all
						# (implicit) property definitions of its superclasses.
						meth_table.add null
					end
				else
					abort
	...
	private fun do_type_coloring: Collection[MType] do
		...
		for mtype in live_cast_types do
			if mtype.is_subset then
				# Subsets’ colors are treated separately.
...
class SeparateCompilerVisitor
	...
	do
		...
		var mtype1 = value1.mtype.as_data_type
		var mtype2 = value2.mtype.as_data_type
		var t1 = value1.mcasttype.as_data_type
		var t2 = value2.mcasttype.as_data_type
	...
	do
		...
		if mtype.need_anchor then
			...
			var recv_type_info = self.type_info(recv)
			self.require_declaration(mtype.const_color)
			return self.new_expr("NEW_{mclass.c_name}((int){length}, {recv_type_info}->resolution_table->types[{mtype.const_color}])", mtype)
		end
		compiler.undead_types.add(mtype)
		self.require_declaration("type_{mtype.c_name}")
		return self.new_expr("NEW_{mclass.c_name}((int){length}, &type_{mtype.c_name})", mtype)
	end
